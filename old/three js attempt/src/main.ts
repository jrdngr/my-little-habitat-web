...
function init() {
	...
	let renderer = new Renderer(500, 500, document.getElementById("display-area"), gridSize, "white");
	grid = new Grid(renderer, gridSize, gridSize, false);
...
function mainLoop(renderer: Renderer) {
	requestAnimationFrame(this.mainLoop.bind(this, renderer));

	for (let i = 0; i < grid.squares.length; i++) {
		let square = grid.squares[i].rendererCell;
	}
